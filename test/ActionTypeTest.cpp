...
using AT = landlock::action::FsAction;
...
TEST_CASE("ActionType::construction")
{
	...
	{
		AT copy = landlock::reduce<
			landlock::ActionRuleType,
			landlock::ActionRuleType::PATH_BENEATH>(
			landlock::action::INVALID_ACTION
		);
		copy = atype;
		CHECK(copy.type_code() == atype.type_code());
		CHECK(copy.min_abi() == atype.min_abi());
...
TEST_CASE("ActionType::combination")
{
	...
	const AT other_action{other_type, other_abi};
	...
	SECTION("operator|")
	{
		const AT new_action = atype | other_action;
